		...
		private HashMap<String, Snowball> shooters = new HashMap();
		  ...
		  @EventHandler
		   public void onPlayerInteract(PlayerInteractEvent e) {
			   ...
			   if (e.getItem().getType() == Material.DIAMOND_BARDING) {
				 if (e.getItem().getItemMeta().getDisplayName().equalsIgnoreCase("§aMelon §dSniper")) {
			      ...
			      scheduler.scheduleSyncDelayedTask(Main.pl, new Runnable()
			      {
			        public void run()
			        {
			          Sniper.cooldown.remove(p.getName());
			        }
			      }, 40L);
		      ...
		      Snowball s = (Snowball)e.getPlayer().launchProjectile(Snowball.class);
			    ...
			    s.setVelocity(p.getEyeLocation().getDirection().normalize().multiply(5));
			    s.setCustomName("Sniper");
		      ...
		      droppedItem.setVelocity(p.getEyeLocation().getDirection().normalize().multiply(6));
		...
		@EventHandler
		  public void onEntityDamage(EntityDamageByEntityEvent e)
		  {
		    ...
		    if (e.getDamager() instanceof Snowball) {
		    	if(e.getDamager().getCustomName().equalsIgnoreCase("Sniper")) {
		      e.setDamage(99.0D);
